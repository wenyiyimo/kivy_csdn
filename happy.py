...
def draw_text():
    ...
    t.color('red')
    t.pensize(2)
    t.pencolor("red")  # 定义画笔颜色为白色，其实就是雪花为白色
    t.pu()  # 提笔，pu=penup
    t.setx(-300)  # 定义x坐标，随机从-350到350之间选择
    t.sety(-350)  # 定义y坐标，注意雪花一般在地上不会落下，所以不会从太小的纵座轴开始
    t.pd()
    t.write("张潇涵", font=("隶书", 50,  "bold"))
    heart_size(0, -300, 2)
    t.pu()
    t.setx(100)  # 定义x坐标，随机从-350到350之间选择
    t.sety(-350)
    t.pd()
    t.write("张潇涵", font=("隶书", 50,  "bold"))


def heart_size(x, y, size):
    ...
    t.pu()
    circle1 = t.clone()
    circle1.goto(x-sqrt(size*size*160)/2, y)
    circle2 = t.clone()
    circle2.goto(x+sqrt(size*size*160)/2, y)
    square = t.clone()
    square.shape("square")
    square.setheading(45)
    square.goto(x, y-sqrt(size*size*160)/2)
    circle1.showturtle()
    circle2.showturtle()
    square.showturtle()
